...
function getVapi() {
    if (!vapi) {
        if (!VAPI_API_KEY) {
            console.error('NEXT_PUBLIC_VAPI_API_KEY environment variable is not set');
        ...
        if (!ASSISTANT_ID) {
            ...
            throw new Error('NEXT_PUBLIC_ASSISTANT_ID environment variable is not set');
        }
        console.log('Initializing Vapi with API key and assistant ID:', ASSISTANT_ID);
...
export function useVapi(book: IBook) {
        ...
        const handlers = {
            ...
            error: (error: any) => {
                ...
                if (errorMessage.includes('ejection') || errorMessage.includes('ejected')) {
                    setLimitError('Session was ended by the server. Click the mic to start again.');
                ...
                } else if (errorMessage.includes('ended') || errorMessage.includes('meeting has ended')) {
                    setLimitError('Session ended. Click the mic to start again.');
                } else {
    ...
    const start = useCallback(async () => {
        ...
        try {
            ...
            if (!ASSISTANT_ID) {
                throw new Error('ASSISTANT_ID is not configured. Please set NEXT_PUBLIC_ASSISTANT_ID environment variable.');
            }

            console.log('Starting voice session with assistant:', ASSISTANT_ID);
            console.log('Voice configuration:', getVoice(voice));
            ...
            console.log('Starting Vapi call with first message:', firstMessage);
            ...
            if (errorMessage.includes('ASSISTANT_ID')) {
                setLimitError('Configuration error: Assistant ID is not set. Please contact support.');
            } else if (errorMessage.includes('ejection') || errorMessage.includes('ejected')) {
                setLimitError('Session was rejected by the server. Please check your configuration.');
            } else {
                setLimitError(`Failed to start voice session: ${errorMessage}`);
            }
        }
